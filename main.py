...
ANSWERS = {
    ...
    'хуби': 'Хубам, раҳмат! 🤖. Шумо чихеlen',
    'sozi': 'Хубам, раҳмат! 🤖. Шумо чихеlen',
    'сози': 'Хубам, раҳмат! 🤖. Шумо чихеlen',
...
if __name__ == "__main__":
    ...
    bot.infinity_polling(timeout=20, long_polling_timeout=10)        text = message.text.lower()
        if "http" in text or "t.me" in text or "@" in text:
            return True
...
# Функсия барои 24 соат блок (Restrict) кардани корбар
def restrict_user_24h(chat_id, user_id):
    until_date = int(time.time()) + 86400  # 24 соат бо сония
    permissions = telebot.types.ChatPermissions(
        can_send_messages=False, can_send_media_messages=False,
        can_send_audios=False, can_send_documents=False,
        can_send_photos=False, can_send_videos=False,
...
def filter_messages(message):
    ...
    # 1. Корбарони оддӣ: Нест кардани ссылка ё никнейм (@) ва додани 3 ОГОҲӢ
    if has_link_or_nickname(message):
        try:
            bot.delete_message(chat_id, message.message_id)
            ...
            if user_warnings[user_id] == 1:
                ...
                restrict_user_24h(chat_id, user_id)
                bot.send_message(
                    chat_id, 
                    f"🚫 Корбар {user_name} қоидаро 3 бор шикаст ва барои спам ба муҳлати 24 соат блок шуд! Пас аз 24 соат худкор кушода мешавад."
                )
                user_warnings[user_id] = 0 # Тоза кардани счётчик пас аз блок
        except Exception as e:
            print(f"Хатогӣ дар антиспам: {e}")
        return

    # 2. Корбарони оддӣ: Нест кардани калимаҳои бад
    if message.content_type == 'text' and has_bad_words(message.text):
        try:
            bot.delete_message(chat_id, message.message_id)
            bot.send_message(chat_id, f"🚫 Корбар {user_name}, калимаҳои манъшуда манъ аст!\nАДМИН: @ALI_UC_SHOPP ❤️")
        except Exception as e:
            print(f"Хатогӣ дар тозакунии калимаҳо: {e}")
        return
