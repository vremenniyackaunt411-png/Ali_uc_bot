...
def get_answers(chat_id):
    ...
    return {
        ...
        'narx': f'нархо барои харидани юс {admin} navisen ❤️',
...
def handle_text_buttons(message):
    ...
    if message.text == "🏠 Менюи Асосӣ":
        ...
        if not active_groups:
            bot.send_message(user_id, "ℹ️ Дар база ягон гурӯҳ ёфт нашуд, ки adminҳояшро навсозӣ кунам.")
            return
        ...
        # ID-и худи ботро мегирем, то онро ҳамчун админи оддӣ сабт накунем
        ...
        for gr_id, gr_title in list(active_groups.items()):
            try:
                ...
                for admin in admins:
                    # ИСЛОҲ: Бот набояд худаш ва Соҳиби ботро ҳамчун админи оддӣ ба база сабт кунад
                    if admin.user.username and admin.user.id != ADMIN_ID and admin.user.id != bot_id:
        ...
        bot.edit_message_text(
            ...
            text=f"✅ **Навсозӣ бо муваффақият анҷом ёфт!**\n\n"
                 ...
                 f"Акнун бот никнейми худашро ба ҷои админҳо сабт намекунад! 🔥"
        )
...
def registration_text_inputs(message):
    ...
    if step == "waiting_for_username":
        ...
        text = (
            ...
            f"1. Тӯгмаи аввали болоро пахш карда, ботро ба гурӯҳи худро илова кунед ва ба он ҳуқуқи **Администратор** диҳед.\n"
    ...
    if has_link_or_nickname(message):
        try:
            ...
            if user_link_warnings[user_id] == 1:
                ...
                bot.send_message(
                    chat_id, 
                    f"🚫 Корбар {user_name} қоидаро 3 бор шикаст ва барои спам ба муҳлати 24 соат блок шуд! Пас аз 24 соат худкор кушода мешается."
                )
                user_link_warnings[user_id] = 0
        except Exception as e:
